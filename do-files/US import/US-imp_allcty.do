...
eststo US_pc_9519_2: reghdfe dlnprice brw l.dlnprice dlnGDP dlnNER, a(group_id) vce(cluster cty_code HS2)
eststo US_pc_0006_1: reghdfe dlnprice brw if year>=2000 & year<=2006, a(group_id) vce(cluster cty_code HS2)
eststo US_pc_0006_2: reghdfe dlnprice brw l.dlnprice dlnGDP dlnNER if year>=2000 & year<=2006, a(group_id) vce(cluster cty_code HS2)
...
binscatter dlnprice brw, xtitle("Monetary policy shocks") ytitle("{&Delta} log price") savegraph("D:\Project E\figures\US_HS6_imp_brw_raw.png") replace nq(30) text(0.04 0.1 "{&beta}=0.132", color(red) place(e))

binscatter dlnprice brw, control(dlnGDP dlndollar l.dlnprice) xtitle("Monetary policy shocks") ytitle("{&Delta} log price") savegraph("D:\Project E\figures\US_HS6_imp_brw_control.png") replace nq(30) text(0.04 0.14 "{&beta}=0.098", color(red) place(e))
