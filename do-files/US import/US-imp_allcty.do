...
collapse (sum) quantity=con_qy1_yr value=con_val_yr, by(year HS6)
...
eststo USall_0019_1: reghdfe dlnprice brw dlndollar, a(product_id) vce(cluster year product_id)
eststo USall_0019_2: reghdfe dlnprice brw l.dlnprice dlndollar, a(product_id) vce(cluster year product_id)
eststo USall_0006_1: reghdfe dlnprice brw dlndollar if year>=2000 & year<=2006, a(product_id) vce(cluster year product_id)
eststo USall_0006_2: reghdfe dlnprice brw l.dlnprice dlndollar if year>=2000 & year<=2006, a(product_id) vce(cluster year product_id)
