...
esttab sum_stats using "tables\tables_Feb2024\sum_stats.tex", replace cells("mean(fmt(2)) sd(fmt(2)) p50(fmt(2)) p25(fmt(2)) p75(fmt(2))") label booktab nonumber nomtitles
...
binscatter dlnprice_YoY brw, xtitle(US monetary policy shock) ytitle(China's export price change) title("Monthly US MPS and China's Export Price") savegraph("D:\Project E\tables\tables_Feb2024\brw_monthly.png") replace discrete
...
esttab forward_* using tables\tables_Feb2024\dynamic.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
eststo lag_4: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_5: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_6: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_7: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_8: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l8.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_9: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l8.brw l9.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_10: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l8.brw l9.brw l10.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_11: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l8.brw l9.brw l10.brw l11.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
eststo lag_12: reghdfe dlnprice_YoY brw l.brw l2.brw l3.brw l4.brw l5.brw l6.brw l7.brw l8.brw l9.brw l10.brw l11.brw l12.brw l12.lnrSI l.dlnprice_YoY dlnNER_US, a(firm_id) vce(cluster firm_id)
...
esttab value_* quant_* using tables\tables_Feb2024\value_quant.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
esttab scaledmps_* using tables\tables_Feb2024\scaledmps.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("FFR" "FFR" "NS" "NS" "Acosta" "Acosta" "JK" "JK") order(ffr_shock ns target path mp cbi)
...
matrix C = r(C)
esttab matrix(C) using tables\tables_Feb2024/correlation_matrix.csv, replace compress b(2)
...
esttab altagg_* using tables\tables_Feb2024\altagg.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(brw L.*)
...
esttab single_* using tables\tables_Feb2024\single.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
esttab SOE_* DPE_* MNE_* JV_* using tables\tables_Feb2024\ownership.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("SOE" "SOE" "DPE" "DPE" "MNE" "MNE"  "JV" "JV")
...
esttab exposure_* using tables\tables_Feb2024\exposure.csv, replace b(3) se(3) noconstant nogap star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress order(brw *brw*)
...
esttab FE_* cluster_* using tables\tables_Feb2024\altFE.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps

*-------------------------------------------------------------------------------

* A8. Fixed and floating regime
...
esttab RMB_* using tables\tables_Feb2024\RMB.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("monthly" "monthly" "monthly" "monthly" "annual" "annual" "annual" "annual")
...
esttab control_* using tables\tables_Feb2024\control.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("CN CPI" "CN Value Added" "CN M2 growth" "US CPI" "US PPI" "VIX" "Input Price" "Oil Price" "All") order(brw *_cn m2g* *_us *vix *S12*)
...
esttab app1_* app2_* using tables\tables_Feb2024\approximate.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("+- 1 month" "+- 1 month" "+- 1 month" "+- 1 month" "+- 2 months" "+- 2 months" "+- 2 months" "+- 2 months")
...
esttab  weightm_* weighty_* using tables\tables_Feb2024\dateweight.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
graph export tables\tables_Feb2024\brw_month_country_20.png, as(png) replace
...
* Trade credit (exporter)
eststo Apay_exp: reghdfe D.Apay brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)
eststo Arec_exp: reghdfe D.Arec brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)
eststo NArec_exp: reghdfe D.NArec brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)
...
eststo NArec_nonexp: reghdfe D.NArec brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)

estfe Apay_exp Arec_exp Apay_nonexp Arec_nonexp, labels(firm_id "Firm FE")
esttab Apay_exp Arec_exp Apay_nonexp Arec_nonexp using tables\tables_Feb2024\TrCredit_A.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps

*-------------------------------------------------------------------------------

* 5. Trade credit (interaction)
...
eststo liquid_3: reghdfe D.Liquid brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)
eststo liquid_4: reghdfe D.Turnover brw L.lnrSI L.Debt, a(firm_id) vce(cluster firm_id)

estfe liquid_*, labels(firm_id "Firm FE")
esttab liquid_* using tables\tables_Feb2024\liquid_A.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
eststo int_FPC_1: reghdfe dlnprice_YoY c.brw#c.l12.FPC_liquid_cic2, a(firm_id time) vce(cluster firm_id)
eststo int_FPC_2: reghdfe dlnprice_YoY c.brw#c.l12.FPC_liquid_cic2 l12.lnrSI l.dlnprice_YoY, a(firm_id time) vce(cluster firm_id)
...
estfe int_FPC_* int_Cash_* int_Liquid_*, labels(firm_id "Firm FE" time "Year-month FE")
esttab int_FPC_* int_Cash_* int_Liquid_* using tables\tables_Feb2024\liquid_B.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(c.brw*)

*-------------------------------------------------------------------------------

* 8. Borrowing cost (first stage)
...
esttab borrow_* debt_* using tables\tables_Feb2024\borrow_A.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps

*-------------------------------------------------------------------------------

* 9. Borrowing cost (interaction)
...
esttab int_IEoL_* int_IEoCL_* int_FNoL_* int_FNoCL_* using tables\tables_Feb2024\borrow_B.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(c.brw*)

*-------------------------------------------------------------------------------

* B2. Trade credit, liquidity and borrowing cost changes with lag interaction

cd "D:\Project E"
use samples\cie_credit_brw,clear
keep if exp_int>0

* Trade credit (lag, exporter)
eststo lag_Apay_exp: reghdfe D.Apay c.brw#c.l.Apay L.lnrSI L.Debt if exp_int>0, a(firm_id year) vce(cluster firm_id)
eststo lag_Arec_exp: reghdfe D.Arec c.brw#c.l.Arec L.lnrSI L.Debt if exp_int>0, a(firm_id year) vce(cluster firm_id)

cd "D:\Project E"
use samples\cie_credit_brw,clear
keep if exp_int==0

* Trade credit (lag, non-exporter)
eststo lag_Apay_nonexp: reghdfe D.Apay c.brw#c.l.Apay L.lnrSI L.Debt if exp_int==0, a(firm_id year) vce(cluster firm_id)
eststo lag_Arec_nonexp: reghdfe D.Arec c.brw#c.l.Arec L.lnrSI L.Debt if exp_int==0, a(firm_id year) vce(cluster firm_id)

estfe lag_A*_exp lag_A*_nonexp, labels(firm_id "Firm FE" year "Year FE")
esttab lag_A*_exp lag_A*_nonexp using tables\tables_Feb2024\TrCreditlag.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(*brw*)
...
eststo lag_liquid_2: reghdfe D.Cash c.brw#c.l.Cash L.lnrSI L.Debt, a(firm_id year) vce(cluster firm_id)
eststo lag_liquid_3: reghdfe D.Liquid c.brw#c.l.Liquid L.lnrSI L.Debt, a(firm_id year) vce(cluster firm_id)
...
esttab lag_liquid_* using tables\tables_Feb2024\liquid_lag.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(*brw*)
...
esttab lag_borrow_* lag_debt_* using tables\tables_Feb2024\borrow_lag.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
esttab decomp_* using tables\tables_Feb2024\decomposition.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps
...
esttab firm_markup_2 within_markup_* across_markup_* using tables\tables_Feb2024\markup.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(*brw*)
...
esttab material wage material_int wage_int imp_int using tables\tables_Feb2024\other.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps order(brw c.brw*)
...
esttab ordinary_* process_* processint_* assembly_* assemblyint_* using tables\tables_Feb2024\process.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("Ordinary" "Ordinary" "Processing" "Processing" "Processing" "Processing" "Assembly" "Assembly" "Assembly" "Assembly")

*-------------------------------------------------------------------------------

* 11. Homogenous vs differentiated good
...
esttab Rauch_* using tables\tables_Feb2024\Rauch.csv, replace b(3) se(3) noconstant star(* 0.1 ** 0.05 *** 0.01) indicate(`r(indicate_fe)') compress nogaps mtitle("Conservative" "Conservative" "Conservative" "Conservative" "Liberal" "Liberal" "Liberal" "Liberal") order(brw c.brw*)
